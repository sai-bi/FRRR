...
module UpdateManager
      ...
      conn.query \
        "replace into
         users  (user_id,
                 name,
                 network)
         values (#{friend[:user_id]},
                '#{conn.escape friend[:name]}',
                '#{conn.escape friend[:network]}')"
  ...
  def update_albums cookie, conn
    ...
    users.each_with_index(symbolize_keys: true) do |user, i|
      ...
      albums.each_with_index do |album, i|
        conn.query \
          "replace into
           ...
           values (#{user[:user_id]},
                   #{album[:album_id]},
                  '#{conn.escape album[:title]}',
                   #{album[:private] ? 'true' : 'false'})"
      end
  ...
  def update_photos cookie, conn, path
    ...
    albums.each_with_index(symbolize_keys: true) do |album, i|

      # Caution ----------------------------------------------------------------
      #
      # A `false` of MySQL type BOOL has value 0 and it is casted into Ruby
      # value by mysql2 as is, but in Ruby only `nil` and `false` can be counted
      # as `false`!
        ...
        conn.query \
          "replace into
           photos (user_id,
                   ...
                   downloaded,
                   cached_faces)
           values (#{album[:user_id]},
                   #{album[:album_id]},
                   #{photo[:photo_id]},
                  '#{conn.escape photo[:caption]}',
                  '#{conn.escape photo[:url]}',
                   #{downloaded},
                  '')"
...
def test
  ...
  UpdateManager::update_friends cookie, conn
  UpdateManager::update_albums  cookie, conn
  UpdateManager::update_photos  cookie, conn, '/media/Passport/frrr/renren-photos'
end
